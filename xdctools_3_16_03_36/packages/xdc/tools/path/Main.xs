...
function run(cmdr, args)
{
    ...
    if (inst.pathFlag) {              /* echo xdc path or -P* args */
        ...
        if (xdcPkgCmd != "") {
            ...
            if (ecode) {
                if (status.output != "") {
                    java.lang.System.err.println("error: the command '"
                        + xdcPkgCmd + "' failed (exit status = " + ecode +
                        "): " + status.output);
                    status.output = "";
                }
            }
...
/*
 *  @(#) xdc.tools.path; 1, 0, 0,148; 11-20-2009 13:27:24; /db/ztree/library/trees/xdctools/xdctools-d08x/src/
 */
