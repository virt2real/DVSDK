...
function run(cmdr, args)
{
    ...
    if (inst.infoFlag || inst.namesFlag) {
        if (inst.arrFlag) {       /* return an array */
            inst.resArr = [];
            _packageInfo(inst, ARRAY_OUTPUT);
            return (inst.resArr);
        }
            ...
            _packageInfo(inst, XML_OUTPUT);
            return (inst.resObj.$xml());
        }
        else {
            inst.resStr = "";
            _packageInfo(inst, STRING_OUTPUT);
            return (inst.resStr);
        }
    }
        ...
        if (inst.bundleRepositoryArg == null) {
            ...
            if (destDir == null) {
                ...
                status = "Destination repository "
                    ...
                    + NOT_FOUND_STR;
                inst.cmdr.warning(status);
                return (status);
            }
            _extractPackages(inst, java.io.File(destDir).getCanonicalPath(),
                             !inst.forceFlag);
        }
        else {
            if (inst.args.length > 1) {
                _extractPackagesFromBundle(inst);                
            }
            else {
                _extractRepositoryFromBundle(inst);
            }
        }
        return (status);
    }
        ...
        if (hasWildcards) {
            ...
            for each (var p in pkgArr) {
                for each (var a in inst.args) {
                    if (_containsWildcards(a)) {
                        if (_wildMatch(a, p)) {
                            _copyPackage(inst, srcDir, destDir, p);
                        }
                    }
                    else if (("" + a) == ("" + p)) {
                        _copyPackage(inst, srcDir, destDir, p);
        ...
        return (_saveRepositoryInfo(inst, inst.fileArg, inst.pathArg,
                                    inst.args));
    }
        ...
        if (java.lang.String(reposList).startsWith(ERROR_FLAG_STR)) {
            return (("" + reposList).substr(ERROR_FLAG_STR.length));
        }
        ...
        if (inst.rrestoreFlag) {
            ...
            if (repoIds.length > 0) {
                if (repoIds.length != inst.args.length - 1) {
                    return (REPO_INDEX_MISMATCH_STR);
                }
                reposList = _mapRepoIds(reposList, repoIds);
                if (java.lang.String(reposList).startsWith("error:")) {
                    return (("" + reposList).substr(ERROR_FLAG_STR.length));
        ...
        for each (var i = 0; i < reposArr.length; i++) {
            ...
            if (curStatus.length > 0) {
                if (java.lang.String(curStatus).startsWith(ERROR_FLAG_STR)) {
                    return (("" + curStatus).substr(ERROR_FLAG_STR.length));
                }
                else {
        ...
        for each (var repoDir in repos) {
            ...
            if (java.io.File(repoDir + '/' + REPOXML_FILE).exists()) {
                setRepoProviderId(repoDir, inst.provIdArg);
            }
            else {
                createRepoXmlFile(repoDir, 
                                  {providerId : inst.provIdArg,
                                   producerId : '',
                                   writable   : 'true'});
...
function _deletePackage(inst, dir, pkgName)
{
    ...
    if (java.io.File(pkgXdcFile).exists()) {    /* found package directory */
            ...
            do {
                ...
                if (!files.length) {
                    ...
                    try {
                        ...
                        inst.cmdr.warning(CANNOT_DELETE_FILE_STR +
                                pkgParentFile.getCanonicalPath());
                    }
        ...
        inst.cmdr.warning(pkgName + " not found in repository " +
                                     java.io.File(dir).getCanonicalPath());
    }
}
...
function _extractPackageDependencies(inst, arArr, pkgDeps, destDir)
{
    ...
    for each (var req in pkgDeps) {
        ...
        if (!found) {
            inst.cmdr.warning("Dependency " + req.pkgName +
                                         "[" + req.version + "]" +
                                         NOT_FOUND_STR);
        }
    }
...
function _extractPackages(inst, destDir, check)
{
    ...
    if (pkgArr.length == 0 ||
        ...
        if (notFoundArr.length) {
            inst.cmdr.warning("No packages extracted. Could not find: " 
                             + notFoundArr);
            return;
        }
        ...
        var pkgFiles = _resolvePkgFileArgs(inst, srcDir, pkgArr);
        if (pkgFiles == null) {
            return;
        }
        ...
        for each (var p in pkgFiles) {
            if (isURL(p)) {
                ...
                if (!tarFile.exists()) {
                    inst.cmdr.warning("Cannot find " + tarName);
                    return;
                }
            }
            ...
            if (scanRet == null) {
                ...
                inst.cmdr.warning(
                    "Failed to retrieve package properties for\n" + tarName +
                    ":\n" + scanRet);
                    return;
            }
        }
    ...
    if (check) {
        ...
        if (results.length > 0) {
            inst.cmdr.warning(results.join("\n"));
            return;
        }
    }
    ...
    if (pkgDeps.length > 0) {
        _extractPackageDependencies(inst, arArr, pkgDeps, destDir);
    }
}
...
function _extractRepositoryFromBundle(inst)
{
    ...
    if (destDir == null) {
        ...
        inst.cmdr.warning("Destination repository " +
                java.io.File(destDir).getCanonicalPath() +
                NOT_FOUND_STR);
        return (false);
    }
    ...
    if (isURL(bundleFile)) {
        if (!isValidURL(bundleFile)) {
            inst.cmdr.warning("Bundle " + bundleFile + NOT_RESOLVED_STR);
            return (false);
        }
    }
    else {
    ...
    var status = _extractRepository(inst, srcDir, inst.args[0],
...
function _extractRepository(inst, srcDir, bundleFile, repoName, destDir)
{
    ...
    if (fileType != TAR_FILETYPE && fileType != TARGZ_FILETYPE &&
        ...
        inst.cmdr.warning("Invalid bundle file: " + 
                java.io.File(bundleFile).getCanonicalPath());
        return (false);
    }
    ...
    if (repoEntry != null) {
        ...
        inst.cmdr.warning("Repository " + repoName + "/ not found in " +
                          bundleFile);
        return (false);
    }
}
    ...
    if (pkgsMatchArr.length == 0 ||
                ...
                if (inst.infoFlag) {
                    ...
                    if (output == XML_OUTPUT) {
                        ...
                        if (output == ARRAY_OUTPUT) {
                            ...
                            str += PRODUCER_ID_STR + repoXmlInf.producerId + '\n';
        ...
        for each (var p in pkgsMatchArr) {
            if (_containsWildcards(p)) {
                ...
                /* TODO: complain if a match is not found */
            }
            else {
                pkgFiles.push(p);
            }
        ...
        for each (var p in pkgFiles) {
            if (isURL(p)) {
                ...
                tarName = tarFile.getCanonicalPath();
                if (!tarFile.exists()) {
                    inst.cmdr.warning(tarName + " does not exist");
                    continue;
                }
            }
            ar = Packages.xdc.tools.repoman.Archive();
...
function _resolvePkgFileArgs(inst, srcDir, argsArr)
{
    ...
    if (error) {
        inst.cmdr.warning("Cannot resolve " + a);
        return (null);
    }
        ...
        for each (var f in files) {
            if (!f.isDirectory()) {
                try {
                    ...
                    inst.cmdr.warning(CANNOT_COPY_FILE_STR +
                                                 f.getCanonicalPath());
                }
            }
            else {
                if (f.listFiles().length) {
                    if (!java.io.File(f.getCanonicalPath() + '/' +
                        ...
                        _copyPackageDirectoryContents(inst, f, 
    ...
    if (notFoundPkgs.length > 0) {
        for each (var p in archivedPkgs) {
            ...
            if (_isABundleArchive(inst, pkg)) {
                ...
                searchPkgs = _getArchivedBundlePackagesInfo(inst, pkg, tmpdir);
...
function createRepoXmlFile(repo, attrs)
{
    ...
    try {
        ...
        doc.@producerId = attrs.producerId;
...
function getRepoXmlItems(repo)
{
    ...
    if (java.io.File(filename).exists()) {
        try {
            ...
            producerId = String(doc.@producerId);
            if (!producerId) {
                producerId = '';
            }
                                   ...
                                   destDir)
{
    ...
    if (pkgEntry != null) {
        ...
        _closeInputStreams(streams);
        _copyPackage(inst, tmpdir, destDir, pkgName)
        var tmpdirFile = java.io.File(tmpdir);
        _deleteDirectoryContents(inst, tmpdirFile);
        tmpdirFile['delete']();
    }
    else {
        inst.cmdr.warning("Package " + pkgName + " not found");
    }
}
...
function _extractPackagesFromBundle(inst)
{
    ...
    if (fileType != TAR_FILETYPE && fileType != TARGZ_FILETYPE &&
        ...
        inst.cmdr.warning("Invalid file: " + bundleFile);
        return;
    }
    ...
    if (hasWildcards) {  /* must test all packages in repository */
        ...
        if (extractPkgs.length == 0) {
            inst.cmdr.warning("Failed to find any matching packages");
            return;
        }
    }
    else {
    ...
    for each (var p in extractPkgs) {
        _extractPackageFromBundle(inst, bundleFile, fileType, repoName, p,
                                  destDir);
    }
}
...
/*
 *  @(#) xdc.tools.repoman; 1, 0, 0, 0,147; 11-20-2009 13:27:53; /db/ztree/library/trees/xdctools/xdctools-d08x/src/
 */
