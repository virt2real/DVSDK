 ...
 *  ======== genHdr ========
 */
function genHdr( node, left, up, right )
{
    if (node.name() == 'decl') {
        PL('<a class="xdocAnchor" id="' + node.@anchor + '"></a>');
        if (node.@kind == 'enum') {
        ...
        PL('<a class="xdocAnchor" id="' + String(node.@name).replace(/ /g, '_') + '"></a>');
    }

    PL('<table class="xdocHdr"><tr>');
    PL('<td class="xdocHdrTitle">' + _mkSlug(node, up) + '</td>');
...
function genPrefix( title, summary )
{
    ...
    PL('<link rel="stylesheet" type="text/css" href="' + Gbl.root() + 'xdoc.css" />');
    PL('</head>');
 ...
 *  ======== genSpecFile ========
 */
function genSpecFile( node, slug )
{
    genLabel('<tt>' + Gbl.span('xdoc-id', 'XDCspec') + '</tt> ' + slug, 'sourced in ' +  _mkSpecFile(node));
}
...
function genToc( node )
{
    ...
    if (node.name() == 'unit') {
        PL('<li class="xdocToc"><a class="xdocToc" href="#xdocElems">Individual elements</a></li>');
    }

    if (Gbl.hasDoc(node)) {
        PL('<li class="xdocToc"><a class="xdocToc" href="#xdoc-desc">Additional documentation</a></li>');
        if (node.docSect.length() > 1) {
            var k = 1;
            for each (var ds in node.docSect) {
                PL('<li class="xdocToc2"><a class="xdocToc" href="#xdoc-sect-' + (k++) + '">' + String(ds.@name).toUpperCase() + '</a></li>');
            }
        }
    }
 ...
 *  ======== _mkSlug ========
 */
function _mkSlug( node, scope )
{
    if (node.name() == 'group') {
        return Gbl.span('xdocSynSum', String(node.@name));
    }
...
//    res += node.@readonly == '1' ? Gbl.span('xdoc-kw1', 'readonly ') : '';
    ...
    if (scope) {
        res += Gbl.span('xdoc-id', scope.@name + '.');
    }
    res += node.@name == 'create' ? Gbl.span('xdoc-kw1', 'create') : Gbl.span('xdoc-id', node.@name);
    res += node.@kind == 'fxn' ? '( )' : '';

    if (scope && (node.@kind == 'fxn' || node.@kind == 'config')) {
        res += Gbl.sprintf('$1<small><span class="xdocSynSum">//&nbsp;$2</span></small>', Gbl.spaces(2), node.@instance == '1' ? 'per-instance' : 'module-wide');
    }
