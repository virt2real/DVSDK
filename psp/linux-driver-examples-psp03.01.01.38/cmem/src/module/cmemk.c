...
/* cut-and-paste above as part of adding support for more than 2 blocks */
    ...
    #ifdef NOCACHE
            printk("ioremap_nocache(%#lx, %ld)=%#lx\n", block_start[bi], length, block_virtp[bi]);
    #else
            printk("ioremap_cached(%#lx, %ld)=%#lx\n", block_start[bi], length, block_virtp[bi]);
    #endif
        ...
        if (heap_size[bi] > 0) {
            ...
            printk(KERN_INFO "no remaining memory for heap, no heap created "
                   "for memory block %d\n", bi);
...
/*
 *  @(#) ti.sdo.linuxutils.cmem; 2, 2, 0,115; 6-11-2009 17:57:46; /db/atree/library/trees/linuxutils/linuxutils-e02x/src/
 */
