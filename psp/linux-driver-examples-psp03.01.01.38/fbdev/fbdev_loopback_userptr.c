...
static int display_frame(char id, void *ptr_buffer)
{
	...
	src = ptr_buffer;
	for (y = 0; y < yres; y++) {
		memcpy(dst, src, (test_data.vid1_width * 2));
		dst += line_length;
		src += src_line_len; /* (720 * 2); */
	}
...
static int init_vid1_device(int fd, struct fb_var_screeninfo *var_info)
{
	...
	if (ioctl(fd_vid1, FBIO_SETPOSY, test_data.vid1_ypos) < 0) {
		printf("\nFailed  FBIO_SETPOS");
		return FAILURE;
	}
...
void menu(void)
{
	printf("Usage: fbdev_loopback_userptr [OPTIONS]\n");
	printf("Options:\n");
	...
	printf("\nNotes:\n");
	printf("This application depends on the Linux bootargs to be setup "
			"such that\nosd0 and vid1 windows are framebuffer "
			"windows and are configured with\nenough buffers. "
			"For example to support 720p capture and display:\n\n"
			"video=davincifb:vid0=OFF:vid1=1280x720x16,5400K:"
			"osd0=1280x720,3600K:osd1=0\n");
	...
	printf("\nNote: this program requires cmem kernel module to be "
			...
			" cmem module is to install DVSDK (3.10 onwards)\n"
			" Insert the cmemk module into the kernel using\n"
			"insmod cmemk.ko phys_start=0x83C00000 "
			"phys_end=0x88000000 pools=7x1843200\n");
}
