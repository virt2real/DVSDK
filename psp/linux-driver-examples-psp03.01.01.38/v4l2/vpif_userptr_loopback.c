...
**+--------------------------------------------------------------------------+**
*******************************************************************************/
/* File Name   : saUserptrLoopback.c
 * Description : This is the sample to show DavinciHD capture and display
 * 		 functionality using user pointer buffers. Capture detects
 * 		 input and standard. The same output and standard
 * 		 is set in the display. It allocates 3 driver buffer for
 * 		 capture and display side using VDCE driver. It captures frames
 * 		 from the captures, copies them from capture buffer to the
 * 		 display buffer and displays them.
		...
		v4l2_std_id *std)
{
	...
	*capture_fd = open((const char *)capture_device, mode);
	...
	if (*std & V4L2_STD_1080P_60)
		printf("1080P60 Mode Detected %llx\n", *std);
...
int main(int argc, char *argv[])
{
	...
	while(counter < MAXLOOPCOUNT) {
		...
		printf("CaptureBuffer Timestamp %d %d\n", buf1.timestamp.tv_sec, buf1.timestamp.tv_usec);
