/****************************************************************
 *  TI DM644x Serial Loading and Flashing Utilities             *
...
files are
  sfh_DM644x.exe  -  Serial Flashing Host
  slh_DM644x.exe  -  Serial Loading Host
Running
=======
...
the .Net Framework 2.0 or later installed.  
  ...
  sfh_DM644x.exe [options]
  slh_DM644x.exe [options]
	
- Under Linux -
They can also be run on a Linux machine with the latest open-source Mono
Framework installed.

	mono sfh_DM644x.exe [options]
  mono slh_DM644x.exe [options]
...
executables.  These target portions are loaded to the DM644x device via the UART
boot mode.
...
1) Load a UBL or small UBL-like application to the DM644x IRAM
	
	slh_DM644x.exe -load2IRAM <UBL binary file>
	
2) Load a larger image compiled to run at start of DDR space 
	
	slh_DM644x.exe -load2DDR <binary application file>
...
1) Erase the NAND flash
	...
	sfh_DM644x.exe -nanderase
	
2) Flash the NAND with a UBL and u-boot image
	
	sfh_DM644x.exe -nandflash  <UBL binary file> <binary application file>
  
NOTE: The entry point of the UBL is assumed to be 0x0100, but this can be changed by using 
...
4) Flash the NOR flash with a bootable image (typically u-boot)

  sfh_DM644x.exe -norflash  <binary application file>
	
NOTE: If using the DVEVM platform, make sure the J4 jumper is set to the appropriate 
device (NAND or FLASH).  The S3 boot switched must also be set for UART boot mode and
the right bus width - for NOR flash S3[1..4] = 1110, for NAND S3[1..4]=1100.  After
flashing is complete, the switches need to be changed to boot from the flash - for NOR 
flash S3[1..4]=1010, for NAND S3[1..4]=0000.
