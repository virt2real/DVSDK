/* --------------------------------------------------------------------------
  ...
  DESC	      : CCS-based utility to flash the DM35x in preparation for 
                NAND booting
 ----------------------------------------------------------------------------- */
...
static Uint32 nandwriter()
{
  ...
  DEBUG_printString("Starting DM35x NANDWriter.\r\n");
...
NAND_WRITE_RETRY:
  ...
  if (ubl ==1)
  {
     if (NAND_writePage_ubl_header(hNandInfo, blockNum, pageNum, gNandTx) != E_PASS)
     {
       blockNum++;
