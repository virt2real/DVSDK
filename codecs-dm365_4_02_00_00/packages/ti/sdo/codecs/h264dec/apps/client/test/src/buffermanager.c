...
XDAS_UInt8 *lumaGlobalBufferHandle = NULL;
XDAS_UInt8 *chromaGlobalBufferHandle = NULL;
...
*        global buffer array and returns the address of that element and also
*        marks it as BUFFMGR_BUFFER_USED. 
*        In case, if none of the elements are free then it returns NULL
*
*
...
/*****************************************************************************/
/**
*@func BUFFMGR_QueryFreeBuffer()
*@brief  TI's (Texas Instrument) implementation of buffer manager 
*       re-initialization module
*
*        The BUFFMGR_QueryFreeBuffer function searches for a free buffer in the 
...
*/
/*****************************************************************************/
BUFFMGR_buffEleHandle BUFFMGR_QueryFreeBuffer()
{
    XDAS_UInt32 tmpCnt;
    /* Since buffers are re-initialised later on its perfectly possible to 
     * have buffer elements with NULL as the buffer pointers - as soon as 
