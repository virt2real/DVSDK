...
#ifdef ENABLE_CACHE
extern void Host_DCache_Flush(void *hdvicpHandle);
#endif //ENABLE_CACHE
...
int EDMA_registerResource(int lch)
{
	return(1);
}

int EDMA_unregister(int lch, int nParams)
{
	return(1);
}

void * MEMUTILS_getPhysicalAddr(void *ptr)
{
	return(ptr);
}

#ifdef ENABLE_CACHE
void MEMUTILS_cacheWb(void *ptH264Enc, int dummy)
{
	Host_DCache_Flush(ptH264Enc);
}

void MEMUTILS_cacheWbInv(void *ptH264Enc, int dummy)
{
	Host_DCache_Flush(ptH264Enc);
}
void MEMUTILS_cacheInv(void *ptH264Enc, int dummy)
{
	Host_DCache_Flush(ptH264Enc);
}
#else
/*dummy functions in case of cachec is disabled*/
void MEMUTILS_cacheWb(void *ptH264Enc, int dummy)
{
	return;
}
