...
XDAS_UInt32 dec_process = 0;
XDAS_UInt32 peak_dec_process = 0;

XDAS_UInt32   delta = 0;

TimerUtilObj sAudDecTimer;
TimerUtilHandle hAudDecTimer = &sAudDecTimer;
...
/*****************************************************************************/
#define PARAMFILE "g711_dec_params.txt"
...
extern SPHDEC1_Handle decHandle;

G711DEC_ITTIAM_Params G711DEC_ITTIAM_PARAMS1 = {
    {
        sizeof(G711DEC_ITTIAM_Params),
        ISPEECH1_PCM_COMPAND_ALAW,   /* compandingLaw   */
        0,      /* packingType: Not used                */
...
#define IA_G711_DEC_ERROR_ACTION(ps_err_string) \
...
/*****************************************************************************/
/*                                                                           */
/*  Function name : ceapp_decodeBuf                                          */
/*                                                                           */
/*  Description   : Actual decoding of the stream                            */
/*                                                                           */
...
/*                                                                           */
/*  Processing    : Actual decoding of the stream                            */
/*                                                                           */
/*  Outputs       : None                                                     */
/*                                                                           */
...
XDAS_Int32 ceapp_decodeBuf(XDAS_Int32 argc, XDAS_Int8 *argv[])
{
    ...
    G711DEC_ITTIAM_Params  params;

    XDM1_SingleBufDesc inBuf, outBuf;
    G711DEC_ITTIAM_InArgs inArgs;
    G711DEC_ITTIAM_OutArgs outArgs;

    G711DEC_ITTIAM_DynamicParams dynParams;
    G711DEC_ITTIAM_Status    status;
...
#endif
    XDAS_Int32  g711_decode_input_size, g711_decode_output_size, frame_length = 160;
    ...
    params = G711DEC_ITTIAM_PARAMS1; /*  Set to default parameters */
    ...
    ret_status = SPHDEC1_control(decHandle, XDM_GETSTATUS, (ISPHDEC1_DynamicParams *)&dynParams,
        (ISPHDEC1_Status *)&status);

    if (ret_status != SPHDEC1_EOK) {
        printf("CEapp-> Speech Decoder control FAILED, ret_status = %d\n", (int)ret_status);
        ceapp_exit();
        return(0);
    }
    ...
    if(outBuf.bufSize >= 64)
    {
        ...
        ret_status = SPHDEC1_control(decHandle, XDM_GETVERSION,(ISPHDEC1_DynamicParams*)&dynParams,
            (ISPHDEC1_Status*)&status);
        if (ret_status != SPHDEC1_EOK)
        {
    ...
    g711_decode_input_size  = frame_length>>1;
	...
	do{
		...
		fread_bytes = fread(inBuf.buf, 1, g711_decode_input_size, g_pf_inp);
		g711_decode_output_size = fread_bytes<<1;
		...
		if(fread_bytes <= 0)
		{
			...
			ret_status = SPHDEC1_control(decHandle, XDM_FLUSH, (ISPHDEC1_DynamicParams *)&dynParams,
				(ISPHDEC1_Status *)&status);

			if (ret_status != IALG_EOK)
			{
				IA_G711_DEC_ERROR_ACTION("CEapp-> Speech Decoder control FAILED\n Input over\n");
				break;
			}
		...
		ret_status = SPHDEC1_process(decHandle, &inBuf, &outBuf,
		...
		if (ret_status != SPHDEC1_EOK) {
			IA_G711_DEC_ERROR_ACTION("CEapp-> SPHDEC1_process() failed\n");
			return(0);
		}

		fwrite(outBuf.buf, 1, g711_decode_output_size, g_pf_out);
		fflush(g_pf_out);
...
int main(int argc, char** argv)
{
    ...
    else
    {
        ...
        if(err_code == 0)
            ceapp_decodeBuf(argc - 1, (XDAS_Int8 **)&argv[1]);
